...
def solution(N: int, A: list[int]) -> list[int]:
    counters: list[int] = [0] * N
    max_value:int = 0
    increase_value: int = 0

    for X in A:
        if X == N+1:
            increase_value = max_value
        elif 1 <= X <= N:
            counters[X-1] = max(counters[X-1],increase_value)
            counters[X - 1] += 1
            max_value = max(max_value, counters[X-1])

    for index, val in enumerate(counters):
        counters[index] = max(val, increase_value)
